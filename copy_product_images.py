...
# Admin view to copy across files/urls to other matchingwines
...
def copy_bottle_image_to_sister_wines(request, wine_id):
	...
	if wine.wine_code in PROBLEM_WINE_CODES:
		return HttpResponse("Invalid wine code.")
	else:
		ws = Wine.objects.filter(wine_code=wine.wine_code)
	
	if ws:
	  # Look for file or url
	  try:
  	...
  	if not f:
	  	if wine.image_url == DEFAULT_IMAGE:
		  	return HttpResponse("No url available.") 
		  else:
			  url = wine.image_url
	  ...
	  if f:		
		  for w in ws:
			  ...
			  if not w.has_proper_image():
				  ...
				  w.save()
				  updated += 1
		  return HttpResponse("%d wines updated with image file. <a href='%s'>Return to admin.</a>" % (updated,'/admin/'))
  	elif url:
	  	for w in ws:
			  ...
			  if not w.has_proper_image():
				  ...
				  w.save()
				  updated += 1
	  	return HttpResponse("%d wines updated with image url. <a href='%s'>Return to admin.</a>" % (updated,'/admin/'))
	  else:
